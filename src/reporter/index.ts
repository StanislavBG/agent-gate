...
function statusLabel(g: GateResult): string {
  if (g.status === 'skipped') return SKIP;
...
function formatDetails(details: Record<string, unknown>): string {
  ...
  if (Array.isArray(details.models_found) && details.models_found.length > 0) {
    parts.push(`models: ${(details.models_found as string[]).join(', ')}`);
  }
...
export function printReport(result: RunResult, opts: { json?: boolean } = {}): void {
  if (opts.json) {
...
export function exitWithVerdict(result: RunResult, noFail = false): never {
