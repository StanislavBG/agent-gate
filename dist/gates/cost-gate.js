 ...
 */
export async function runCostGate(config, cwd) {
    ...
    for (const usage of usages) {
        if (allowlist.length > 0 && !allowlist.includes(usage.model)) {
            ...
            violations.push(`Model '${usage.model}' not in allowlist (found in ${usage.file}:${usage.line})`);
        }
    }
...
function scanFile(filePath, usages) {
    let content;
    try {
...
//# sourceMappingURL=cost-gate.js.map