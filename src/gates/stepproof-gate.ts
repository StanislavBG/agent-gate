...
import type { GateResult, StepproofConfig } from '../types/index.js';

const execFileAsync = promisify(execFile);

export async function runStepproofGate(
  config: StepproofConfig,
  cwd: string
): Promise<GateResult> {
  ...
  // Run each scenario
  const results: Array<{ file: string; passed: boolean; output: string; error?: string }> = [];
